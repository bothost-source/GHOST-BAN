...
function loadPremiumDB() {
    ...
    try {
        return JSON.parse(fs.readFileSync(PREMIUM_DB_PATH));
    } catch {
        return { premium: [], resellers: [] };
    }
}

function savePremiumDB(data) {
    fs.writeFileSync(PREMIUM_DB_PATH, JSON.stringify(data, null, 2));
}
...
function isPremium(userId) {
    ...
    return db.premium.includes(userId.toString());
}

function addPremium(userId) {
    const db = loadPremiumDB();
    if (!db.premium.includes(userId.toString())) {
        db.premium.push(userId.toString());
        savePremiumDB(db);
        return true;
    }
    return false;
...
function getPremiumList() {
    return loadPremiumDB().premium;
}

// ========== END PREMIUM DATABASE ==========
...
async function startWhatsAppBot(phoneNumber, telegramChatId = null) {
  ...
  if (telegramChatId && !sock.authState.creds.registered) {
    // Wait for socket to stabilize before requesting pairing code
    setTimeout(async () => {
      try {
        const code = await sock.requestPairingCode(phoneNumber, 'GHOST-BAN');
        ...
        if (telegramBot) {
          telegramBot.sendMessage(telegramChatId, 
            "❌ Pairing Failed\n\n" +
            "📱 Number: " + phoneNumber + "\n" +
            "❌ Error: " + error.message + "\n\n" +
            "━━━━━━━━━━━━━━━━━━━\n" +
...
`;
      ...
      const opts = {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "📱 Pair", callback_data: "pair_info" }],
...
✨ *Available Commands:*
 ...
 /ban - execute ban protocol
 /addprem - add user (owner cmd)
 /delprem - delete user (owner cmd)
 /listprem - list users (owner cmd)
━━━━━━━━━━━━━━━━━━━
💡 Developed by @LORDTARRIFIC
`;
...
`;
      ...
      case "addprem": {
        const premUserId = msg.text.split(' ')[1];
        ...
        if (addPremium(premUserId)) {
          ...
          try {
            telegramBot.sendMessage(premUserId, 
              "🎉 PREMIUM ACTIVATED\n\n" +
          ...
          telegramBot.sendMessage(chatId, 
            "⚠️ ALREADY PREMIUM\n\n" +
            "User " + premUserId + " is already premium.",
            { parse_mode: 'HTML' }
          );
        }
        break;
      ...
      case "delprem": {
        const delPremUserId = msg.text.split(' ')[1];
        if (!isAdmin(msg.from.id)) {
          telegramBot.sendMessage(chatId, 
            "⛔ ACCESS DENIED\n\n" +
            "❌ Only bot owner can use this command.",
        ...
        if (removePremium(delPremUserId)) {
          telegramBot.sendMessage(chatId, 
            "✅ PREMIUM REMOVED\n\n" +
            "👤 User: " + delPremUserId + "\n" +
            "🗑️ Status: Premium revoked",
            { parse_mode: 'HTML' }
          );
          try {
            telegramBot.sendMessage(delPremUserId, 
              "🔒 PREMIUM REVOKED\n\n" +
          ...
          telegramBot.sendMessage(chatId, 
            "⚠️ NOT PREMIUM\n\n" +
            "User " + delPremUserId + " is not in premium list.",
            { parse_mode: 'HTML' }
          );
        }
        break;
      ...
      case "listprem": {
        ...
        if (premList.length === 0) {
          ...
          let premText = "📋 PREMIUM USERS (" + premList.length + ")\n\n";
          for (let i = 0; i < premList.length; i++) {
            premText += (i + 1) + ". <code>" + premList[i] + "</code>\n";
          }
          telegramBot.sendMessage(chatId, premText, { parse_mode: 'HTML' });
        }
        break;
      }
...
// Self-ping every 2 minutes to prevent Render from sleeping
const RENDER_URL = process.env.RENDER_EXTERNAL_URL || `http://localhost:${PORT}`;
setInterval(() => {
    http.get(RENDER_URL, (res) => {
        console.log(`🏓 Self-ping: ${res.statusCode} at ${new Date().toLocaleTimeString()}`);
    }).on('error', (err) => {
        console.log(`⚠️ Self-ping failed: ${err.message}`);
    });
}, 2 * 60 * 1000); // 2 minutes

console.log(`🔄 Self-ping active: every 2 minutes to ${RENDER_URL}`);
